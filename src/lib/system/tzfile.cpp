...
static unordered_map<string, tzinfo> setup_tz_files() {
	...
	if (!FileSystem::checkFileAccess(path, FileSystem::exists)) {
...
static bool tzhead_read(tzhead *tz_head, FILE *file) {
	...
	if (UNLIKELY(fread(tz_head, sizeof (tzhead), 1, file) != 1) || UNLIKELY(memcmp(&tz_head->tzh_magic, TZ_MAGIC, sizeof (tz_head->tzh_magic)) != 0)) {
		return false;
	}
...
TimeZone *mint::timezone_read(FILE *file, size_t size, char **extra, size_t extra_size) {
	...
	if (tz_head->tzh_version != '\0') {
		...
		long int to_skip = (tz_head->tzh_timecnt * (4 + 1)
							+ tz_head->tzh_typecnt * 6
							+ tz_head->tzh_charcnt
							+ tz_head->tzh_leapcnt * 8
							+ tz_head->tzh_ttisstdcnt
							+ tz_head->tzh_ttisgmtcnt);
	...
	if (trans_width == 8) {
		...
		if (UNLIKELY(rem < 0 || size_t(rem) < size_t(tz_head->tzh_timecnt * (8 + 1) + tz_head->tzh_typecnt * 6 + tz_head->tzh_charcnt))) {
			return nullptr;
		}
	...
	if (UNLIKELY(fread(tz->tz_transitions, trans_width, tz_head->tzh_timecnt, file) != tz_head->tzh_timecnt) || UNLIKELY(fread(tz->tz_typeidxs, 1, tz_head->tzh_timecnt, file) != tz_head->tzh_timecnt)) {
		return nullptr;
	}
...
static bool tzrule_parse_tzname(tzrule *tz_rules, const char **specs, int whichrule) {
	...
	if (length < 3) {
		...
		while (('a' <= *cptr && *cptr <= 'z') || ('A' <= *cptr && *cptr <= 'Z') || ('0' <= *cptr && *cptr <= '9') || *cptr == '+' || *cptr == '-') {
			++cptr;
		}
...
static bool tzrule_parse_rule(tzrule *tz_rules, const char **specs, int whichrule) {
	...
	if (*cptr == 'J' || isdigit(*cptr)) {
		...
		if (sscanf(cptr, "M%hu.%hu.%hu%n", &rule->m, &rule->n, &rule->d, &consumed) != 3
				|| rule->m < 1 || rule->m > 12
...
static void compute_change(tzrule *rule, int year) {
	...
	if (year > EPOCH_YEAR) {
		t = ((year - EPOCH_YEAR) * 365
			 + ((year - 1) / 4 - EPOCH_YEAR / 4)
			 - ((year - 1) / 100 - EPOCH_YEAR / 100)
			 + ((year - 1) / 400 - EPOCH_YEAR / 400)) * SECS_PER_DAY;
	}
	else {
		t = 0;
	}
...
found:
		tz_names[tz->tz_types[tz->tz_typeidxs[i - 1]].tt_isdst] = &tz->tz_zonenames[tz->tz_types[tz->tz_typeidxs[i - 1]].tt_desigidx];
...
leap:
	...
	if (leap_found) {
		...
		if (timer == tz->tz_leaps[i].transition && ((i == 0 && tz->tz_leaps[i].change > 0) || tz->tz_leaps[i].change > tz->tz_leaps[i - 1].change)) {

			leap_hit = 1;

			while (i > 0 && tz->tz_leaps[i].transition == tz->tz_leaps[i - 1].transition + 1 && tz->tz_leaps[i].change == tz->tz_leaps[i - 1].change + 1) {
				++leap_hit;
				--i;
			}
		}
...
static long int ydhms_diff(long int year1, long int yday1, int hour1, int min1, int sec1, int year0, int yday0, int hour0, int min0, int sec0) {
...
string mint::timezone_default_name() {
	...
	if (name == nullptr) {
		...
		string path = FileSystem::symlinkTarget("/etc/localtime");
		...
		while (iteration-- > 0 && !path.empty() && (index = path.find("/zoneinfo/")) == string::npos) {
			path = FileSystem::symlinkTarget(path);
		}
	...
	if (name == nullptr) {
		if (FileSystem::checkFileAccess("/etc/timezone", FileSystem::exists)) {
