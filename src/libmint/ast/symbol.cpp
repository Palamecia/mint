...
Symbol::Symbol(const string& symbol) :
	m_hash(make_symbol_hash(symbol.data(), symbol.length())),
...
bool Symbol::operator ==(const Symbol &other) const {
	return m_hash == other.m_hash && !strcmp(m_symbol, other.m_symbol);
}
...
bool Symbol::operator <(const Symbol& other) const {
	...
	if (UNLIKELY(m_hash == other.m_hash)) {
		return strcmp(m_symbol, other.m_symbol) < 0;
	}
...
uint64_t Symbol::make_symbol_hash(const char *symbol, size_t length) {
